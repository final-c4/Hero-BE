...
public interface PersonnelAppointmentRepository extends JpaRepository<PersonnelAppointment, Long> {
    ...
    /**
     * 발령일이 특정 날짜(오늘)보다 이전이거나 같으면서, 상태가 대기 중인 모든 발령 건을 조회합니다.
     *
     * @param appointmentDate 기준 날짜 (보통 오늘)
     * @param status 상태
     * @return 처리되지 않은 발령 예약 목록
     */
    List<PersonnelAppointment> findAllByAppointmentDateLessThanEqualAndStatus(LocalDate appointmentDate, String status);
}
